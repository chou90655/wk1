	...
	<view class="content">
		...
		<scroll-view scroll-y="true" class="record-list">
			<view v-for="(item, index) in records.list" :key='index' class="record-item">
				<uni-row>
					<uni-col :span="12">
						<text class="panel-info-label">用户:</text>
				...
				<uni-row>
					<uni-col :span="8">
						<text class="panel-info-label">充值姓名:</text>
						<text class="panel-info-value">{{item.payname}}</text>
					</uni-col>
					<uni-col :span="16">
						<text class="panel-info-label">充值方式:</text>
						<text class="panel-info-value">{{item.paytypetitle}}</text>
					</uni-col>
				</uni-row>
				<uni-row>
					<uni-col :span="16">
					...
					<uni-col :span="8">
						<text class="panel-info-label">状态:</text>
						<text class="panel-info-value">{{state[item.state]}}</text>
					</uni-col>
				</uni-row>
			</view>
		</scroll-view>
...
<script>
	...
	export default {
	    ...
	    methods:{
			...
			getRecords(){
				api.getDepositRecord(this.formData).then(res=>{
					if(res.code == 0){
