	...
	<view class="content">
		...
		<scroll-view scroll-y="true" class="record-list">
			<view v-for="(item, index) in records.list" :key='index' class="record-item">
				<uni-row>
					<uni-col :span="12">
						<text class="panel-info-label">提款账户:</text>
						<text class="panel-info-value">{{item.accountname}}</text>
					</uni-col>
					<uni-col :span="12">
				...
				<uni-row>
					<uni-col :span="24">
						<text class="panel-info-label">提款方式:</text>
						<text class="panel-info-value">{{item.bankname}}-{{item.bankbranch}}</text>
					</uni-col>
				</uni-row>
				<uni-row>
					<uni-col :span="16">
...
<script>
	...
	export default {
	    ...
	    methods:{
			...
			getRecords(){
				api.getWithdrawalRecord(this.formData).then(res=>{
					if(res.code == 0){
