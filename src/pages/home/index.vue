<template>
	<view class="content">
		<view class="home-head">
			<uni-row>
				<uni-col :span="10">
					<img-cache style="width: 235rpx; height: 74rpx;padding: 10rpx;" src='../../static/logoV.png'></img-cache>
				</uni-col>
				<uni-col :span="11" v-if="null!=user" style="text-align: right;padding-top: 20rpx;">
		...
		<view class='home-gg uni-row uni-flex'>
			<view class="text" style="width: 40rpx;">
				<img-cache style="width: 27rpx;height: 27rpx;margin: 0 10rpx ; margin-top: 25rpx;" src="../../static/icon/voice.png"></img-cache>
			</view>
			<view class="text-gg" @click="gotoNew(ggBin)">
				<uni-notice-bar color="#8A8989" backgroundColor='#fff' :scrollable="true" :single="true" :text="getGgInfo()" />
			</view>
			<view class="text" style="width: 200rpx;margin-top: 6rpx;">
				<button type="primary" size="mini" @click="gotoNewslist()">更多公告</button>
			</view>
		</view>
			...
			<uni-row class="uni-row uni-flex">
				<uni-col v-for="(item, index) in tabTools" :key='index'>
					<view class="tab-tools" @click="gotoPages(item)">
						<img-cache class="tab-tools" :src="item.icon" @click="gotoPages(item)"></img-cache>
					</view>
		...
		<view class="uni-flex uni-row home-games">
			<view class="home-sidebar">
				<view class="tab-bar-btn" v-for="(item, index) in sideBars" :key='index' :class="{active:index == sideBarIndex}" @click="switchSideBar(index)">
					<view class="sidebar-img-bg">
						<img-cache class="sidebar-img" :src="index == sideBarIndex?item.icon:item.iconHL"></img-cache>
					</view>
			...
			<view class="home-content" v-if='sideBarIndex == 0'>
				<view class="home-gamges">
					<uni-row>
						<uni-col class="game-content" :span="6" v-for="(item, index) in games" :key='index'>
							<view v-if="item.id > 0"  @click='gotoBet(item)'>
								<view v-if="item.title.length > 2">
									<p class="game-title-main">{{item.title.substr(0, 2)}}</p>
									<p class="game-title-desc">{{item.title.substr(2, item.title.length - 2)}}</p>
								</view>
								<view v-if="item.title.length <= 2">
									<p class="game-title">{{item.title}}</p>
								</view>
								<img-cache class="game-icon" :src='item.iconUrl' mode="aspectFit"></img-cache>
							</view>
						</uni-col>
					</uni-row>
				</view>
				<view class="home-lotty">
					<view style="margin-left: 20rpx; margin-top: 10rpx;">
						<view style="display: inline-flex;">
							<img-cache class="lotty-img" :src="sideBars[sideBarIndex].icon"></img-cache>
						</view>
						<view class="lotty-title">{{sideBars[sideBarIndex].title}}大厅</view>
					</view>
					<view class="lotty-enter" @click="gotoGames()">
						全部彩种
						<view style="display: inline-block; margin-left: 10rpx;">
							<img-cache style="width: 8rpx; height: 14rpx;" src="../../static/icon/right.png"></img-cache>
						</view>
					</view>
				</view>
			</view>
			<scroll-view v-if='sideBarIndex !== 0 && sideBarIndex !== 4' class="home-content" :scroll-y="true">
				<uni-row>
					<uni-col :span="20" :offset="2" v-for="(item, index) in games" :key='index'>
						<!-- {{item.name}} -->
						<img-cache style="width: 100%" mode="widthFix" :src="item.img" @click='getLogin(item)'></img-cache>
					</uni-col>
				</uni-row>
			</scroll-view>
			<scroll-view v-if='sideBarIndex == 4' class="home-content" :scroll-y="true">
				<uni-row>
					<uni-col :span="10" :offset="2" v-for="(item, index) in games" :key='index'>
						<!-- {{item.name}} -->
						<img-cache style="width: 100%;" mode="widthFix" :src="item.img" @click='getLogin(item)'></img-cache>
					</uni-col>
				</uni-row>
		...
		methods:{
			...
			getGg(){
				api.getGg({
					...
					if(res.code ==0 && null != res.data.root && res.data.root.length > 0){
			...
			getGgInfo(){
				...
				if(undefined != this.ggBin && null != this.ggBin){
					content = this.ggBin.title + '  ' + ak.Utils.getDateTimeStr(new Date(this.ggBin.oddtime* 1000), 'Y-m-d H:i:s');
				}
				return content;
			},
			getApiGameList(){
			...
			getLogin(item){
				api.getLogin({
					...
					if(res.code == 0){
						...
						uni.navigateTo({
						url:'/pages/webview/webview?url=' + encodeURIComponent(res.data.url) + "&title="+ item.type_name// + res.data.name
					})
					}
				})
			}
	...
	.home-games{
		...
		.home-content{
			...
			.home-gamges{
				...
				padding-left: 30rpx;
				.game-content{
					.game-title-main,.game-title-desc,.game-title{
						margin: 12rpx 10rpx;
						font-size: 19rpx;
						font-family: PingFang;
						font-weight: 500;
